...
class Gearing {
   public:
      ...
      vector<Gear> compress (vector<Gear> gteeth) {
         ...
         for (int i=1; i<n; i+=2) upp.push_back(make_pair(gteeth[i].y+gteeth[i].sz*10,i));
         ...
         for (int i=1; i<n; i+=2) low.push_back(make_pair(gteeth[i].y-gteeth[i].sz*10,i));
      ...
      void fitin (vector<int> gteeth) {
         ...
         fprintf(debug,"score2*: %0.4lf\n",scoring(lobest));
         /*
         //compress every degree?
         lobest=compress(lobest);
         */
