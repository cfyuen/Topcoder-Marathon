         ...
         */
         ...
         for (int i=1; i<group.size(); i++) {
             ...
             for (int j=2; j<isol; j+=2) {
                 ...
                 fprintf(debug,"i:%d j:%d, sco:%d area:%0.4lf tot:%0.4lf, best:%0.4lf\n",i,j,nows,(((height-j)*h+20)*(width*10+20)),nows*1.0/(((height-j)*h+20)*(width*10+20)),bestsco);
                 fflush(debug);
             ...
             for (int j=0; j<=width; j++)
                 if (hash[0][i*1000+j+1000001]!=-1) {
                    ...
                    for (int row=i-1; row>=0; row--)
                      ...
                      //fprintf(debug,"%d %d->%d %0.4lf->%0.4lf = %0.4lf -> %0.4lf\n",nowscore,orip,intsco,areanow,((width*10+20)*(max(mr,gain)*h+20)),nowscore*1.0/areanow,mxp);
                      ...
                      //fprintf(debug,"adj:%d(%d) (%d %d) %d   %0.4lf\n",i,hp,mr,mp,intsco-orip,mxp);
         ...
         fprintf(debug,"score (%d):%0.4lf usedtime:%0.4lf\n\n\n",times,tscore,runtime()-lastime);
         times++;
         lastime=runtime();
      } while (lastime<timeout && times<99);
         ret=maxret;
         fprintf(debug,"onetime:%0.4lf\n",onetime);
         fprintf(debug,"\n%d\n",n);
         for (int i=0; i<n; i++)
