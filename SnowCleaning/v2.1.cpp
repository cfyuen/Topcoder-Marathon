	...
	public:
		...
		void placeworker2() {
			...
			for (int i=1; i<cntc; i++) {
				...
				if (c[i].sz <= 5*sal/fine) { // 1 worker
					tar[cntt++] = c[i].cen;
				}
				else if (c[i].sz <= 15*sal/fine) { // 2 worker
					for (int j=0; j<2; j++)
						tar[cntt++] = c[i].corn[j];
				}
				else if (c[i].sz <= 30*sal/fine) { // 4 workers
					for (int j=0; j<4; j++)
						tar[cntt++] = c[i].corn[j];
				}
				else { // 5+ workers
					int left = (c[i].sz-30*sal/fine);
					fprintf(debug,"extra %d: %d\n",c[i].sz,left);
					for (int j=0; j<=left; j+=10)
						tar[cntt++] = c[i].cen;
					for (int j=0; j<4; j++)
						tar[cntt++] = c[i].corn[j];
				}
