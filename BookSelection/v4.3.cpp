...
change findheight, estimate average
...
//delete when submit
FILE *debug,*result;
double tlim1_1=2.0,tlim1_2=4.0,tlim2=5.5;
      ...
      */
      ...
      void minoradjust () {
       ...
       vector<int> majorheight (vector<int> geth,double timelim) {
          ...
          for (int i=0; i<geth.size(); i++) {
              ...
              for (int j=0; j<30 && j<indsco[i].size(); j++) {
                  tots[i]+=indsco[i][j];
                  totcnt[i]++;
                  }
              aversco+=tots[i]*1.0/totcnt[i];
              }
          aversco/=geth.size();
          ...
          vector<int> newh;
          for (int i=0; i<geth.size(); i++)
              if (tots[i]*1.0/totcnt[i]>aversco) {
                 fprintf(debug,"in: %d\n",geth[i]);
      ...
      void select () {
          ...
          while (runtime()<tlim1_2) {
             ...
             for (int i=1; i<getsz; i++)
                 for (int j=0; j<b.size(); j++)
                     if (abs((candh[i]+candh[i-1])/2-b[j].h)<=1 && use[b[j].ind]!=-1 && hcho[b[j].h]==0) {
