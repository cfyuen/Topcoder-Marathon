...
class RockyMine {
	public:
		...
		Coor saferoute (Coor at,int bbid) {
			...
			if (perv.size()>1) {
				...
				for (int i=0; i<pcnt; i++)
					g[pick[i].x][pick[i].y].gold=-g[pick[i].x][pick[i].y].gold;
				ret+=bpick;
			}
		...
		pair<Coor,string> moveto (Coor ori,Coor at,Coor tar,int lim) {
			...
			int sx,sy;
			for (int i=0; i<bfin; i++) {
				sx=fin[i].c.x-2; sy=fin[i].c.y-2;
				for (int xx=max(0,sx); xx<=min(n-1,sx+4); xx++)
					for (int yy=max(0,sy); yy<=min(m-1,sy+4); yy++) {
						g[xx][yy].rock+=b[fin[i].typ].eff[xx-sx][yy-sy];
					}
			}
			for (int i=1; i<bfin; i++) mxmv[i]+=mxmv[i-1];
			for (int i=0; i<n; i++)
				for (int j=0; j<m; j++)
					if (g[i][j].gold<0) g[i][j].gold=-g[i][j].gold;
			*/
			pcs=greedyman();
			ret+=pcs.second;
			fflush(debug);
