...
void updatescore () {
     ...
     if (hull.empty()) {
        if (prev[callnr-1][sucx[callnr-1]/sz][sucy[callnr-1]/sz]==-1) return;
        allsco[prev[callnr-1][sucx[callnr-1]/sz][sucy[callnr-1]/sz]]=10;
        return;
        }
     //**************************************************************************************************************
         ...
         if (inhull==1) {
            ...
            for (int i=0; i<canadd.size(); i++)
                if (hull.find(canadd[i])==hull.end()) {
                   if (canadd.size()<=1) allsco[canadd[i]]+=12;
                      else if (canadd.size()<=3) allsco[canadd[i]]+=8;
    ...
    while (crep<rept+stseed) {
       ...
       if (crep>=stseed) {
          ...
          for (int i=0; i<pass.size(); i++) {
              ...
              for (int j=0; j<18; j++) {
                  for (int k=0; k<18; k++)
                      if (pass[i].second[j][k]=='1') res << '#'; else res << '.';
                  res << endl;
                  }
              res << endl;
              */
