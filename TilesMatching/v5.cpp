...
class TilesMatching {
   public:
      double score1 () {
         ...
         if (space<=s*s*5/6) score=10000*pow((space)*1.0/(s*s*5/6),4.0);
            else score=10000;
         ...
         if (d==nowd) score*=pow(okt/(n*1.0*n),0);
            else score*=pow(okt/(n*1.0*n),((4-nowd)*1.0/3+1)*3.0);
      ...
      double score0 (int rowt) {
         ...
         for (int i=0; i<s; i++)
             for (int j=0; j<s; j++)
                 if (board[i][j].c==-1) space++;
         if (space<=s*s*5/6) score=11500*pow((space)*1.0/(s*s*5/6),3.0);
            else score=10000;
         vector<double> row,col;
         for (int i=0; i<s; i++) {
             int tom=0,bassco=1000;
