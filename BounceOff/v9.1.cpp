...
void pushstate (state st,int minst) {
    ...
    if (j!=n) {
       ...
       for (int i=0; i<st.now.size(); i++)
           if (useded[i]==0) {
              fprintf(stderr,"useless\n");
              st.now.erase(st.now.begin()+i);
              i--;
              }
   ...
   public:
      vector <string> placeObstacles(vector <int> oX, vector <int> oY, int R) {
         ...
         for (int i=1; i<n; i++) {
             ...
             if (curtime>5) firtime=5;
             maxlimit=(int)(pow(12-firtime*2,1.0/i)*5.8);
             if (i>=height) maxlimit=20; 
             pqshade.clear();
             fprintf(debug,"nextpoint: %d\n",i);
             //fprintf(stderr,"nextpoint: %d\n",i);
             state news,tst;
             if (heu[i].empty() && curtime<14) {
                int j;
                for (j=i; j<n; j++)
                    if (!heu[j].empty()) break;
                if (j==n) {
                   i-=4; i=max(0,i);
                   continue;
                   }
             ...
             while (maxlimit>0 && !heu[i].empty()) {
                ...
                if (tst.visit[0]!=-1) {
                   ...
                   if (pqshade.size()>maxlimit) {
                      ...
                      //fprintf(stderr,"sz: %d  tle: %0.4lf\n",pqshade.size(),pqshade[maxlimit-1]);
                   ...
                   if (i<height) {
                   int trylim=max(20,(int)(20+(9+i*6.5/height-curtime)/(height-i-0.3)*80));
