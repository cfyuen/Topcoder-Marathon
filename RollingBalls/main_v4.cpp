...
int bd[65][65],bdlvl[65][65],bdlvlcnt[65][65],bdlvllast[65][65];
double hsco[65][65];
int n,m,cntball;
int lg2[1024];
vector<string> ret;
coor block[MAXBALL][65][65],last[MAXBALL][65][65];
coor ballmap[MAXBALL];
int rballmap[65][65];
int fix[65][65];
coor cfr,cto;
	...
	while (!q.empty()) {
		...
		for (int d=0; d<4; d++) {
			...
			while (bdlvl[nt.x+dx[d]][nt.y+dy[d]] >= 0) {
				if (step+1 < v[nt.x][nt.y] && bdlvl[nt.x+dx[d]][nt.y+dy[d]] < lvl && (bdlvlcnt[nt.x+dx[d]][nt.y+dy[d]] > 1 || bdlvllast[nt.x+dx[d]][nt.y+dy[d]] != ballid)) {
					v[nt.x][nt.y] = step+1;
					q.push(make_pair(-(step+1),nt));
					if (bdlvl[nt.x][nt.y] > lvl) {
						bdlvl[nt.x][nt.y] = lvl;
						bdlvlcnt[nt.x][nt.y] = 1;
						bdlvllast[nt.x][nt.y] = ballid;
						block[ballid][nt.x][nt.y].x = nt.x+dx[d]; block[ballid][nt.x][nt.y].y = nt.y+dy[d];
						last[ballid][nt.x][nt.y] = c;
					}
					else if (bdlvl[nt.x][nt.y] == lvl) {
						bdlvlcnt[nt.x][nt.y]++;
					}
				}
				nt.x += dx[d]; nt.y += dy[d];
...
void calcBoardScore() {
	...
	memset(bdlvlcnt,0,sizeof(bdlvlcnt));
	memset(bdlvllast,-1,sizeof(bdlvllast));
	for (int i=0; i<n+2; i++)
		for (int j=0; j<m+2; j++) {
...
coor lastc[MAXBALL][65][65];
int lastd[MAXBALL][65][65],vb[MAXBALL][65][65];
int stmap[65][65];
int stcnt = 0;
				...
				for (int j=0; j<m+2; j++) {
					...
					if (start[i-1][j-1] == '#') {
						...
						if (start[i-1][j-1] >= '0' && start[i-1][j-1] <= '9') {
							...
							rballmap[i][j] = cntball;
							ballmap[cntball] = coor(i,j);
							cntball++;
						}
