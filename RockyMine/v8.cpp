...
/*********************
v8 Approach:
	1. Rank the gold to take + try different start point
	    a. Gold value
...
class RockyMine {
	public:
		...
		int scorest (const State &st) {
			if (bdt==1) return st.tgc*15-st.pb.size()*250;
		...
		pair<Coor,string> moveto (Coor ori,Coor at,Coor tar,int opt) {
			//Design the least number of moves to go to target
			string ret="",nmv;
			pair<Coor,string> pcs;
			fprintf(debug,"Bfin %d\n",bfin);
		...
		int scofin (Coor at,Coor tar,int lim) {
			...
			fflush(debug);
			if (backup==-1000000) return -1000000;
			if (bestsco==-1000000) return backup;
			return bestsco;
		}
		pair<Coor,string> movetry (Coor ori,Coor tar,int lim,int opt) {
			int locbest=-1000000,locbfin,nowsco,locdist,d1,d2;
			PutBomb locfin[300];
			Coor locat;
			int mindist=10000;
			Coor tat;
			...
			fill(ori);
			d2=reach[tat.x][tat.y];
			if (mindist!=10000 && (mindist-locdist)*3<reach[tat.x][tat.y]-(d2-d1)) {
				nowsco=scofin(tat,tar,lim);
				if (bestsco>=locbest) {
					locbest=bestsco;
					locbfin=bfin;
			...
			Route tr; tr.fr=locat; tr.to=tar;
			rou.push_back(tr);
			...
			for (int i=0; i<bfin; i++)
				fin[i]=locfin[i];
			fprintf(stderr,"Time: %0.4lf\n",runtime()); fflush(stderr);
			return moveto(ori,locat,tar,opt);
		}
		void trial () {
			for (int i=0; i<dn; i++) {
				b[i].left=oleft[i];
			...
			while (ret.length()<=move*3/2 && cont==1) {
				...
				if (cont==0 && ret.length()<=move) bdt=1;
		...
		string real () {
			...
			for (int i=0; i<rou.size(); i++) {
				...
				/*
				if (runtime()>9) scofin(tat,rou[ind].to,20);
					else if (omove>7000) scofin(tat,rou[ind].to,40);
					else if (omove>5000) scofin(tat,rou[ind].to,60);
					else scofin(tat,rou[ind].to,100);
				*/
				if (bdt==1) pcs=movetry(tc,rou[ind].to,100,bdt);
				else {
					scofin(tat,rou[ind].to,10);
					if (runtime()>7.5) pcs=moveto(tc,tat,rou[ind].to,1);
						else pcs=moveto(tc,tat,rou[ind].to,bdt);
				}
				tc.x=pcs.first.x; tc.y=pcs.first.y;
				ret+=pcs.second;
			...
			while (ret.length()<=move && cont==1) {
				...
				if (runtime()>9) pcs=movetry(tc,now,20,1);
					else if (omove>7000) pcs=movetry(tc,now,40,1);
					else if (omove>5000) pcs=movetry(tc,now,60,1);
					else pcs=movetry(tc,now,100,bdt);
				tc.x=pcs.first.x; tc.y=pcs.first.y;
				ret+=pcs.second;
				cont=0;
				for (int i=0; i<dn; i++) {
			...
			for (int i=0; i<dn; i++) {
				...
				for (int j=0; j<5; j++)
					for (int k=0; k<5; k++) {
						...
						if (b[i].eff[j][k]==0) bombcnt-=b[i].left;
					}
			}
			m=W; n=Gold.size()/m;
			for (int i=0; i<n; i++)
