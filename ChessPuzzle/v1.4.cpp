...
double cal_heu (double ret,int nowreach,int net,int xx,int yy,vector<int> dep) {
   //depth SD
   
   double mean=0,SD=0;
   for (int i=1; i<=K; i++) mean+=(depcnt[i]-dep[i])*i;
   mean/=(bd.size()*bd[0].size()-emptysq);
   for (int i=1; i<=K; i++) SD+=(mean-i)*(mean-i)*(depcnt[i]-dep[i]);
   SD/=(bd.size()*bd[0].size()-emptysq);
   //SD=sqrt(SD);
   
   /*
   double mean=0,SD=0;
   for (int i=0; i<=K; i++) mean+=(depcnt[i]-dep[i])*i;
   mean/=(bd.size()*bd[0].size());
   for (int i=0; i<=K; i++) SD+=(mean-i)*(mean-i)*(depcnt[i]-dep[i]);
   SD/=(bd.size()*bd[0].size());
   SD=sqrt(SD);
   */
   //average reachability
   //ret-=nowreach*1.0/(bd.size()*bd[0].size()-emptysq);
   //fprintf(debug,"ret  %0.4lf\n",ret);
   ret=(nowreach+net)*1.0/(bd.size()*bd[0].size()-emptysq);
   ret=ret-SD;
   //fprintf(debug,"%0.3lf = %0.3lf - %0.3lf\n",ret,(nowreach+net)*1.0/(bd.size()*bd[0].size()-emptysq),SD); fflush(debug);
   return ret;
}
   ...
   if (now.lprev>=1 && now.lprev<=4) {
      for (int k=0; k<8; k++) {
          ...
          if (nx>=0 && nx<bd.size() && ny>=0 && ny<bd[0].size() && bd[nx][ny]!=0 && now.v[nx][ny]==0) {
             ...
             tnow.dep[depth[nx][ny]]++;
             tnow.dep[depth[nx][ny]-1]--;
             tnow.heur=cal_heu(now.heur,now.nowreach,-netrea,nx,ny,tnow.dep);
...
pair<int,int> nextmove2a () {
   ...
   int searchdepth=55,maxdepth=0,searchlimit=200,nowlimit[60];
...
class ChessPuzzle {
   public:
      ...
      string click(string revealed) {
          ...
          if (K*bd.size()*bd[0].size()-removed<2*bd.size()*bd[0].size()) res=nextmove2a();
             else res=nextmove();
