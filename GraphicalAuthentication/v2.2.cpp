...
int sz,callnr=0,term,cnttest=0;
...
void exhaust (int tar,int ext,int st) {
     //fout << tar << " " << ext << " " << st << endl;
     if (term==1 || (tar+st>M && tar!=0) || cnttest>10000) return;
     if (tar==0 && ext==0) {
        int match=allinhull();
        /*
        fout << "TEST ";
        ...
        */
        cnttest++;
        if (match==callnr) term=1;
        }
        else if (tar==0) {
        for (int i=st; i<min(50,K); i++) 
            if (term!=1) {
            hull.insert(final[i]);
            exhaust(tar,ext-1,i+1);
            if (term!=1) hull.erase(final[i]);
            }
        }
        else {
        for (int i=st; i<M; i++) 
            if (term!=1) {
            hull.insert(final[i]);
            if (tar==1) exhaust(tar-1,ext,M);
               else exhaust(tar-1,ext,i+1);
            if (term!=1) hull.erase(final[i]);
            }
        }
}
