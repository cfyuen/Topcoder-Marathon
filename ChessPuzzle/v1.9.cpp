      ...
      string click(string revealed) {
          ...
          if (!seq.empty()) {
             ...
             if (K*bd.size()*bd[0].size()-removed==bd.size()*bd[0].size() && runtime()<timelim*2.0/3) {
                ...
                res=nextmoveinit(2,300);
                if (seq.size()>bestseq.size()) {
                   k12heu=2; bestseq=seq;
                   }
             ...
             if (K>1) {
                if (K*bd.size()*bd[0].size()-removed<=max(250,2*(int)bd.size()*(int)bd[0].size())) {
                   ...
                   */
                   if (tn<timelim/3.0) {
                      if (K*bd.size()*bd[0].size()-removed<=120) evalim=250;
                         else if (K*bd.size()*bd[0].size()-removed<=250) evalim=200;
                         else evalim=220-(int)((K+8)*(bd.size()*bd[0].size())/25.0);
                      }
                      else if (tn<timelim*3.0/5) evalim=220-(int)((K+8)*(bd.size()*bd[0].size())/25.0);
                      else if (tn<timelim*2.0/3 && K*bd.size()*bd[0].size()-removed<=150) evalim=220-(int)((K+8)*(bd.size()*bd[0].size())/25.0);
                      else if (tn<timelim*0.9343) {
                              if (K*bd.size()*bd[0].size()-removed>150) evalim=80;
                                 else if (K*bd.size()*bd[0].size()-removed>100) evalim=100;
                                 else evalim=130;
                              }
                      else evalim=75;
