...
class KnightsMoveCipher {
   public:
      ...
      void permanent (string word,int pos,int x,int y,int ind) {
          if (pos==word.length()) {
             for (int i=0; i<n; i++)
                for (int j=0; j<n; j++) {
                    if (cv[i][j]==0) v[i][j]=0;
                    }
             last.insert(make_pair(x,y));
             return;
             }
          ...
          for (int i=1; i<d; i++)
              for (int j=0; j<4; j++) {
                  ...
                  if (nx>=0 && ny>=0 && nx<n && ny<n && cv[nx][ny]==0 && ctext[nx*n+ny]==word[pos]) {
                     cv[nx][ny]=1;
                     permanent(word,pos+1,nx,ny,ind);
                     cv[nx][ny]=0;
                     }
                  }
          }
      ...
      string decipher(string ciphertext, vector <int> Delta) {
         ...
         for (int j=0; j<min((int)append.size(),5); j++)
             printf("%d: %s\n",j,append[j].c_str());
         printf("\n");
         for (int i=0; i<min((int)append.size(),20); i++) {
             int verdot=0;
             ...
             for (int j=0; j<n; j++)
                 for (int k=0; k<n; k++)
                     if (append[i].length()>0 && ctext[j*n+k]==append[i][0]) {
                        cv[j][k]=1;
                        permanent(append[i],1,j,k,0);
                        cv[j][k]=0;
                        }
             int dis;
             if (!last.empty()) {
                dis=delta[append[i].length()-1];
                for (set<pair<int,int> >::iterator it=last.begin(); it!=last.end(); it++)
                    for (int j=1; j<dis; j++)
                        for (int k=0; k<4; k++) {
                            int nx=(*it).first+dx[k]*j,ny=(*it).second+dy[k]*(dis-j);
                            if (nx>=0 && ny>=0 && nx<n && ny<n && v[nx][ny]==0)
                               poss.insert(make_pair(nx,ny));
                            }
                }
                else {
                for (int j=0; j<n; j++)
                    for (int k=0; k<n; k++)
                        poss.insert(make_pair(j,k));
                memset(v,0,sizeof(v));
                }
             vphr.clear();
             accphr.clear();
             ...
             for (int k=0; k<n; k++) {
                for (int j=0; j<n; j++) {
                    if (v[k][j]==1) fprintf(debug,"#");
                       else fprintf(debug,".");
                    }
                fprintf(debug,"\n");
                }
             fprintf(debug,"poss loc:\n");
             ...
             */
             for (int j=0; j<12000; j++) {
                 if (curpos+dict[j].length()<=delta.size()+1 && (dict[j].length()!=1 || j<100) && (dict[j].length()!=2 || j<800) && (dict[j].length()!=3 || j<2000)) {
                    for (set<pair<int,int> >::iterator it=poss.begin(); it!=poss.end(); it++) {
                        int k=(*it).first,m=(*it).second;
                        if (v[k][m]==0 && ((append[i].length()>0 && ctext[k*n+m]=='.') || (append[i].length()==0 && ctext[k*n+m]==dict[j][0]))) {
                           //fprintf(debug,"try: %s\n",dict[j].c_str());
                           v[k][m]=1;
                           cntapp=0;
                           ...
                           if (cntapp>0) {
                              ...
                              if (j<10 && i<3) {
                                 cout << append[i] << endl;
                                 cout << dict[i] << " " << accphr[accphr.size()-1] << endl;
                                 }
                              */
...
int main () {
    ...
    */
    ...
    fclose(alls);
    srand(1234567891);
    int testno=150,sttest=100,nowtest=0;
