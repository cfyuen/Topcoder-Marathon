         ...
         while (totp<n) {
            ...
            */
            if (add==0 || (add<=2 && n>50)) {
               ...
               handle:
               fprintf(debug,"\nADJ (%d)\n",sump);
               vector<int> adjp; int doneadj=1,ther=max(2,(int)sqrt(grp.size()));
               while (grp.size()>15 && doneadj==1) {
                  ...
                  for (int j=0; j<grp.size(); j++)
                      if (final[grp[j]*2]==minx) {
                         adjp.push_back(grp[j]); ok[0]++;
                         }
                  if (ok[0]>ther) { adjp.resize(adjp.size()-ok[0]); ok[0]=-1; }
                  for (int j=0; j<grp.size(); j++)
                      if (final[grp[j]*2]==maxx) {
                         adjp.push_back(grp[j]); ok[1]++;
                         }
                  if (ok[1]>ther) { adjp.resize(adjp.size()-ok[1]); ok[1]=-1; }
                  for (int j=0; j<grp.size(); j++)
                      if (final[grp[j]*2+1]==miny) {
                         adjp.push_back(grp[j]); ok[2]++;
                         }
                  if (ok[2]>ther) { adjp.resize(adjp.size()-ok[2]); ok[2]=-1; }
                  for (int j=0; j<grp.size(); j++)
                      if (final[grp[j]*2+1]==maxy) {
                         adjp.push_back(grp[j]); ok[3]++;
                         }
                  if (ok[3]>ther) { adjp.resize(adjp.size()-ok[3]); ok[3]=-1; }
                  //adjust
                  ...
                  fprintf(debug,"adjp:%d ok:%d%d%d%d\n",adjp.size(),ok[0],ok[1],ok[2],ok[3]);
                  for (int i=0; i<adjp.size(); i++) {
