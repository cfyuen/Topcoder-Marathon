...
class SnowCleaning {
	public:
		...
		void placeworker2() {
			...
			coor tar[505],addtar[105],besttar[105];
			int cntt = 0,perm[105],bestm[105];
			...
			for (int i=1; i<cntc; i++) {
				...
				for (int w=0; w<20; w++) {
					...
					while (cntm < c[i].sz) {
						...
						// add worker
						if (cntp < 100 && (md == inf || md*1LL*c[i].sz/w*fine >= min(50,(2000-day))*sal)) {
							// todo: add new worker
							cost += min(50,(2000-day))*sal;
							md = 0;
							int tr;
							for (tr=0; cm[tr]; tr++);
							bs = tr;
							//printf(debug,"add worker\n");
						}
						else { // mark worker
							mak[bp] = 1;
