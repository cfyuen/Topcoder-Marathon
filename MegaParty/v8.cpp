   ...
   public:
      vector <double> arrangement(vector <int> vA, vector <int> vB, double X1, double X2) {
         ...
         */
         ...
         while (totp<n) {
            ...
            */
            if (add==0 || (add<=2 && n>50)) {
               ...
               fprintf(debug,"ratio:%0.4lf\n",grp.size()*1.0/((maxx-minx+1)*(maxy-miny+1)));
               
               vector<int> adjp,tv; int doneadj=1,ther=max(2,(int)sqrt(grp.size()));
               while (grp.size()>9 && doneadj==1) {
                  minx=2000000; maxx=-2000000; miny=2000000; maxy=-2000000;
                  int minp=2000,ind=0;
                  doneadj=0;
                  for (int j=0; j<grp.size(); j++) {
                      minx=min(minx,final[grp[j]*2]);
         ...
         int isol=((int)(e2/10.0001))+1,nowcol=0,width=0;
         isol=((int)(e2/h))+1;
