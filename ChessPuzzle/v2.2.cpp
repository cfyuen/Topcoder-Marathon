...
int reachsum=0,emptysq=0,k12heu=0;
      ...
      */
      ...
      if (nowlimit[2]==1 && next.size()==0) {
         if (bd[now.ox][now.oy]==4 && bd[now.lx][now.ly]==4 && depth[now.ox][now.oy]>2 && depth[now.ly][now.ly]>1) {
            seq=now.rec;
            //fprintf(stderr,"suddenbreak: (%d,%d) (%d,%d)\n",now.lx,now.ly,now.ox,now.oy); fflush(stderr);
            //double tn=runtime();
            //while (runtime()<tn+10) {}
            break;
            }
...
class ChessPuzzle {
   public:
      string start(int K1, vector <string> board) {
          ...
          if (K==1) {
             double rough=runtime(),endt=0.6-bd.size()*bd[0].size()*1.0/500; int key=0;
             for (int i=0; runtime()+rough<timelim*endt && i<cand.size(); i++) {
                 px=cand[i].second.first; py=cand[i].second.second; prev=bd[px][py];
                 fprintf(stderr,"cand: (%d,%d)   rough: %0.4lf - %0.4lf/%0.4lf\n",px,py,rough,runtime(),timelim*endt); fflush(stderr);
                 nextmoveinit(1,300);
                 fprintf(stderr,"seq: %d\n",bd.size()*bd[0].size()-seq.size());
                 if (seq.size()>bestseq.size()) {
                    bestseq=seq; k12heu=1;
                    }
                 key++;
                 rough=runtime()/key;
                 if (runtime()+rough>timelim*endt) break;
                 px=cand[i].second.first; py=cand[i].second.second; prev=bd[px][py];
                 fprintf(stderr,"cand: (%d,%d)   rough: %0.4lf - %0.4lf/%0.4lf\n",px,py,rough,runtime(),timelim*endt); fflush(stderr);
                 nextmoveinit(0,300); 
                 fprintf(stderr,"seq: %d\n",bd.size()*bd[0].size()-seq.size());
                 if (seq.size()>bestseq.size()) {
                    bestseq=seq; k12heu=0;
                    }
                 key++;
                 rough=runtime()/key;
                 }
      ...
      string click(string revealed) {
          ...
          if (!seq.empty()) {
             ...
             /*
             if (K==1) {
                res=nextmoveinit(k12heu,300);
                if (runtime()<timelim) seq.resize(5);
                res=*seq.begin();
                seq.erase(seq.begin());
                }
             */
             if (K*bd.size()*bd[0].size()-removed==bd.size()*bd[0].size() && runtime()<timelim*2.0/3) {
                vector<pair<int,int> > bestseq;
                res=nextmoveinit(1,300);
                if (seq.size()>bestseq.size()) {
                   k12heu=1; bestseq=seq;
                   }
                res=nextmoveinit(0,300);
                if (seq.size()>bestseq.size()) {
                   k12heu=0; bestseq=seq;
                   }
                fprintf(stderr,"k12heu = %d\n",k12heu);
                //fprintf(stderr,"k12heu = %d %d\n",k12heu,bestseq.size());
                }
             ...
             if (K>=1) {
                if (K*bd.size()*bd[0].size()-removed<=max(250,2*(int)bd.size()*(int)bd[0].size())) {
                   ...
                   if (tn>timelim*9.3/10) {
                      ...
                      if (K*bd.size()*bd[0].size()-removed<indleft-10 || (K==1 && K*bd.size()*bd[0].size()-removed<indleft-2)) {
                   ...
                   fprintf(stderr,"evalim: %d (%0.4lf) %d\n",evalim,tn,K*bd.size()*bd[0].size()-removed);
                    fflush(stderr);
                   toteva+=evalim;
                   if (K*bd.size()*bd[0].size()-removed<=bd.size()*bd[0].size()) res=nextmovefinal(1,55,evalim);
                      else res=nextmovefinal(k12heu,55,evalim);
                   ...
                   */
                   ...
                   if (tn<timelim*2.0/3) evalim=300-(int)(sqrt(K*bd.size()*bd[0].size())*3);
                      else evalim=100;
