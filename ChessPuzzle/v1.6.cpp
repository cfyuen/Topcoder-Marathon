      ...
      for (int k=0; k<8; k++) {
          ...
          if (nx>=0 && nx<bd.size() && ny>=0 && ny<bd[0].size() && bd[nx][ny]!=0 && now.v[nx][ny]==0) {
             ...
             if (heuno==0) tnow.heur=cal_heu(now.heur,now.nowreach,-netrea,nx,ny);
                else tnow.heur=cal_heu1(now.heur,now.nowreach,-netrea,nx,ny);
...
class ChessPuzzle {
   public:
      string start(int K1, vector <string> board) {
          ...
          if (K==1) {
             for (int i=0; runtime()<12.0 && i<cand.size(); i++) {
      ...
      string click(string revealed) {
          ...
          if (!seq.empty()) {
             ...
             if (K*bd.size()*bd[0].size()-removed<=max(300,2*(int)bd.size()*(int)bd[0].size())) {
                ...
                //if (seq.size()>K*bd.size()*bd[0].size()-removed-max(300,2*(int)bd.size()*(int)bd[0].size()))
                //   seq.resize(K*bd.size()*bd[0].size()-removed-max(300,2*(int)bd.size()*(int)bd[0].size()));
                res=*seq.begin();
                seq.erase(seq.begin());
                }
             }
