...
class RockyMine {
	public:
		...
		pair<Coor,string> saferoute (Coor at,int bbid) {
			...
			for (int i=0; i<pcnt; i++) {
				perv.push_back(i);
				//fprintf(debug,"Pick (%d,%d)[%d] - %d\n",pick[i].x,pick[i].y,g[pick[i].x][pick[i].y].gold,g[pick[i].x][pick[i].y].rock); fflush(debug);
			}
			fflush(debug);
			Coor endc=pick[0];
			if (perv.size()>1) {
				string bpick="",curpick="";
				int bpstep=100000,lim=10000;
				do {
					curpick="";
		...
		//2. place a new bomb
		void goldbfs (int fb) {
			int q1,q2,v[2][105][105],tg[105][105],cntt=0,curl,obe=pat.be;
			memset(v,0,sizeof(v));
			...
			v[curl%2][q[0].x][q[0].y]=1;
			while (q1<=q2) {
				q1++;
				if (rec[q1-1].length()!=curl || rec[q1-1].length()==0) {
					memset(v[curl%2],0,sizeof(v[curl%2]));
					string trec=pat.rec+rec[q1-1];
				...
				if (tg[q[q1-1].x][q[q1-1].y]!=-1) {
					...
					if (pat.be<=clos[tg[q[q1-1].x][q[q1-1].y]]) {
						string trec=pat.rec; Coor tc=pat.c;
						pat.rec+=rec[q1-1];
						pat.c=q[q1-1];
				...
				*/
				for (int i=0; i<5; i++) {
					nat.x=q[q1-1].x+dx[i]; nat.y=q[q1-1].y+dy[i];
					if (nat.x>=0 && nat.y>=0 && nat.x<n && nat.y<m && v[1-curl%2][nat.x][nat.y]==0 && forb[fb][nat.x][nat.y]==0) {
						v[1-curl%2][nat.x][nat.y]=1;
						q2++;
						rec[q2]=rec[q1-1]+dir(i);
						q[q2].x=nat.x; q[q2].y=nat.y;
					}
		...
		pair<Coor,string> greedyman () {
			getgoldpos();
			int totg=0,curl=0,curbest=0,lim=5,cnt=0;
			int bhigh=0,bbb=0;
			string bret="";
			Coor bat=fin[0].c;
			/*
			...
			if (runtime()>4 && pcnt>=8) {
				for (int k=0; k<bfin; k++) {
					...
					for (int xx=max(0,sx); xx<=min(n-1,sx+4); xx++)
						for (int yy=max(0,sy); yy<=min(m-1,sy+4); yy++) {
							g[xx][yy].rock+=b[fin[k].typ].eff[xx-sx][yy-sy];
							if (g[xx][yy].rock>0) g[xx][yy].gold=og[xx][yy].gold;
						}
					b[fin[k].typ].left++;
				}
			...
			while (!pq.empty()) {
				...
				if (pat.gv>=totg*9/10) {
					if (pat.bp==pat.be) {
						...
						fprintf(debug,"pq(%d)= (%d,%d) [%d]: %d/%d  %s\n  ",pq.size(),pat.c.x,pat.c.y,pat.gv,pat.bp,pat.be,pat.rec.c_str());
						for (int i=0; i<pcnt; i++)
							fprintf(debug,"%d",(pat.gold[i]+2)/2);
						fprintf(debug,"\n");
						fflush(debug);
						for (int k=pat.bp; k<bfin; k++) {
							int sx,sy;
							sx=fin[k].c.x-2; sy=fin[k].c.y-2;
							for (int xx=max(0,sx); xx<=min(n-1,sx+4); xx++)
								for (int yy=max(0,sy); yy<=min(m-1,sy+4); yy++) {
				...
				int vl=0,sf=0;
				for (int i=pat.bp-8; i<pat.bp; i++) {
					vl*=2;
					if (i>=0) vl+=(pat.gold[i]+2)/2;
				}
				for (int i=pat.rec.length()-5; i<pat.rec.length(); i++) {
					sf*=2;
					if (i>=0 && pat.rec[i]>='0' && pat.rec[i]<='9') sf++;
				}
				//fprintf(debug,"%d,%d,%d,%d (%d)\n",pat.bp,pat.be,vl,sf,cbest[pat.bp][pat.bp-pat.be][vl][sf]);
				if (pat.gv<=cbest[pat.bp][pat.bp-pat.be][vl][sf]) {
					//fprintf(debug,"Byebye\n");
					continue;
				}
				cbest[pat.bp][pat.bp-pat.be][vl][sf]=max(pat.gv,cbest[pat.bp][pat.bp-pat.be][vl][sf]);
				int lost=0;
				for (int i=0; i<pcnt; i++)
					if (pat.gold[i]==-1 && pat.be>clos[i]) lost+=og[pick[i].x][pick[i].y].gold;
				if (lost>totg/10) {
					//fprintf(debug,"Byebye\n");
					continue;
				}
				if (pat.bp==pat.be && pat.gv>bhigh && lost==0) {
					bhigh=pat.gv;
					bbb=pat.bp;
					bret=pat.rec;
					bat=pat.c;
		...
		string real () {
			...
			for (int i=0; i<rou.size(); i++) {
				...
				if (runtime()>9) pcs=moveto(tc,tat,rou[ind].to,20,1);
					else if (omove>7000) pcs=moveto(tc,tat,rou[ind].to,30,1);
					else if (omove>4000) pcs=moveto(tc,tat,rou[ind].to,50,1);
					else if (omove>2000) pcs=moveto(tc,tat,rou[ind].to,70,1);
					else pcs=moveto(tc,tat,rou[ind].to,100,bdt);
				tc.x=pcs.first.x; tc.y=pcs.first.y;
				ret+=pcs.second;
			...
			while (ret.length()<=move && cont==1) {
				...
				if (runtime()>9) pcs=moveto(tc,tat,now,20,1);
					else if (omove>7000) pcs=moveto(tc,tat,now,40,1);
					else if (omove>5000) pcs=moveto(tc,tat,now,60,1);
