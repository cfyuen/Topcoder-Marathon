...
class MegaParty {
   public:
      vector <double> arrangement(vector <int> vA, vector <int> vB, double X1, double X2) {
                ...
                if (v[ppl[i]]==0) {
                   ...
                   for (set<pair<int,int> >::iterator it=can.begin(); it!=can.end(); it++) {
                       ...
                       if (np*1.0/areafac>mxp) {
                          mxp=np*1.0/areafac+1e-8; intp=np;
                          mr=row; mp=pos;
                          }
                       }
         ...
         memset(hash,-1,sizeof(hash));
         int isol=((int)(e2/h))+1,nowcol=0,width=0,height=0,stop,mod=0;
         ...
         int nowscore=0;
         double nowarea=(height*h+20)*(width*10+20);
         for (int i=0; i<n; i++) {
             for (int j=0; j<n; j++) {
                 double ar,ap,tx,ty;
                 ar=h*final[i*2];
         ...
         for (int i=1; i<group.size(); i++) {
             ...
             for (int j=2; j<isol; j+=2) {
                 ...
                 if (nows*1.0/(((height-j)*h+20)*(width*10+20))>bestsco) {
                    bestsco=nows*1.0/(((height-j)*h+20)*(width*10+20))+1e-8;
                        ...
                        for (int pos=0; pos<=width; pos++) 
                          if (hash[0][row*1000+pos+1000001]==-1) {
                            ...
                            if ((nowscore+np-orip)*1.0/((width*10+20)*(max(row,gain)*h+20))>mxp) {
                               mxp=(nowscore+np-orip)*1.0/((width*10+20)*(max(row,gain)*h+20))+1e-8; intsco=np;
                               mr=row; mp=pos;
                               }
                          }
