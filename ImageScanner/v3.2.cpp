				...
				for (int x=0; x<let[(*mit).first.a][(*mit).first.b][(*mit).first.sz].size(); x++) {
					...
					for (int y=0; y<let[(*mit).first.a][(*mit).first.b][(*mit).first.sz][0].length(); y++) 
						if (let[(*mit).first.a][(*mit).first.b][(*mit).first.sz][x][y]=='1') {
							...
							if (ret[(*mit).first.sx+x][(*mit).first.sy+y]=='1') {
								if (scaned[x]==1) pmat+=1;
								//else pmat++;
							}
				...
				if (scaned[i]==1) {
					for (int j=0; j<M; j++)
						if (ret[i][j]=='1' && (j==0 || (j>0 && ret[i][j-1]=='0')) && epn[i][j]==0) {
							...
							bind.a=-1; sind.a=-1;
							for (mit=pref.begin(); mit!=pref.end(); mit++) {
								pn=let[(*mit).first.a][(*mit).first.b][(*mit).first.sz].size();
								pm=let[(*mit).first.a][(*mit).first.b][(*mit).first.sz][0].length();
								if ((*mit).first.sx<=i && (*mit).first.sy<=j && (*mit).first.sx+pn-1>=i && (*mit).first.sy+pm-1>=j) {
									if ((*mit).second>5) {
										fprintf(debug,"  "); ltos((*mit).first);
										fprintf(debug," %d %d (%d)\n",(*mit).first.sx,(*mit).first.sy,(*mit).second);
									}
									if ((*mit).second>best) {
										if (bind.a!=-1 && dif[hash(bind.a,bind.b,bind.sz)][hash((*mit).first.a,(*mit).first.b,(*mit).first.sz)]>20)
											secbest=best,sind=bind;
										best=(*mit).second;
										bind=(*mit).first;
									}
									else if ((*mit).second>secbest && dif[hash(bind.a,bind.b,bind.sz)][hash((*mit).first.a,(*mit).first.b,(*mit).first.sz)]>20) {
										secbest=(*mit).second;
										sind=(*mit).first;
							...
							int noscan=0;
							if (bind.a>=0) {
								for (int k=0; k<let[bind.a][bind.b][bind.sz].size(); k++) {
									if (scaned[bind.sx+k]==0) noscan++;
									else noscan=0;
									if (noscan>=6) {
										break;
									}
							...
							if (noscan<6) {
								if ((best/secbest>=3 && best>bind.sz-8) || (best-secbest>20) || (bind.a!=-1 && best>4 && sind.a==-1)) {
									confirm(bind);
								}
							}
							else if (bind.a!=-1 && sind.a!=-1) updatesco(bind,sind);
						}
				}
			...
			for (int i=0; i<N/3; i++) {
				...
				for (int x=0; x<N; x++)
					if (scaned[x]==0) {
						noscan=0;
						ind=x-1;
						while (ind>=0 && scaned[ind]==0) ind--;
						noscan+=x-ind-1;
						ind=x+1;
						while (ind<N && scaned[ind]==0) ind++;
						//noscan+=ind-x-1;
						if (ind-x-1>=3 && noscan>=3 && ind<N) scansco[x]+=100;
					}
				showboard();
