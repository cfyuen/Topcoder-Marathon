							...
							for (int x=i-3; x<=i+3; x++)
					    		for (int y=j-3; y<=j+3; y++)
					        		if (x>=0 && y>=0 && x<n && y<m && !(x==i && y==j)) {
										curg+=g[x][y].gold*0.6/(abs(x-i)+abs(y-j));
								}
							curb=curr*4.0/bombpow;
							if (cnts<3) curb+=g[i][j].rock/2.0;
							curs+=(int)(curb*12);
							curs+=(int)sqrt(curg);
							double cursco=curg*1.0/curs;
							if (curs<=move-usedmove && curb<=bleft && cursco>bestsco) {
								fprintf(debug,"(%2d,%2d,%d) G %4.2lf R %4d S %4d B %4.1lf %0.4lf\n",i,j,k,curg,curr,curs,curb,cursco);
...
//		}
		...
		int scorest (const State &st) {
			return st.tgc*8-st.tgd-st.pb.size()*max(40,max(n,m))-st.step;
		}
