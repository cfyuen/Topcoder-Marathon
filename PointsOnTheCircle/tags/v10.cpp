...
public:
	...
	void SA() {
		...
		while (iter--) {
			...
			if (iter < totaliter) {
				nor = 0;
				nor += fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand()+fastrand();
				nor -= RAND_MAX*6;
				idd = (int)ceil(abs(nor*temp*fac));
				//if (iter%500 == 0) fprintf(debug,"%d\n",idd);
				idx = fastrand()%N; idy = (idx + idd)%N;
			}
			else {
				idx = fastrand()%N; idy = (idx + 1 + fastrand()%(N-1))%N;
			}
