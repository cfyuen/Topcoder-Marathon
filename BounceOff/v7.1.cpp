...
void pushstate (state st,int minst) {
    ...
    if (j!=n) {
       ...
       //fprintf(stderr,"acc time:%0.4lf\n",st.time); fflush(stderr);
       }
       else {
       //can optimize
...
state aimsim (state nows,int touch) {
     ...
     */
     ...
     while (time < 500 && bounces < 200){
         ...
         double A,B,C,tB,tC,ca,cb,cc,cd,F,G,H,R,S,T,U,I,L,K,M,Q,P;
         for (int i = 0; i<tar.size(); i++) {
             if (visit[i]) continue;
             if (tar[i].y<250 && touch!=n) break;
   ...
   public:
      vector <string> placeObstacles(vector <int> oX, vector <int> oY, int R) {
         ...
         for (int i=1; i<n; i++) {
             ...
             maxlimit=(int)(pow(8,1.0/i)*5.8);
             if (tar[i].y<250) maxlimit=20; 
             pqshade.clear();
