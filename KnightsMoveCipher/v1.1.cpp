...
class KnightsMoveCipher {
   public:
      ...
      void dfs (string word,int pos,int x,int y,int ind) {
         if (pos==word.length()) {
      ...
      string decipher(string ciphertext, vector <int> Delta) {
         ...
         memset(v,0,sizeof(v));
         vector<int> secset,newset;
         for (int i=0; i<27; i++) {
             secset.push_back(1);
             newset.push_back(0);
             }
         for (int w=0; w<=delta.size()-10+1; w++) {//must change when length change
             int reset1=1;
             for (int i=0; i<27; i++) 
                 if (newset[i]==1) { newset[i]=0; reset1=0; }
             if (reset1==1)
                for (int i=0; i<27; i++) secset[i]=1;
             for (int i=0; i<5000; i++) {
                 ...
                 */
                 ...
                 if (dict[i].first[0]=='.' && secset[26]==1) ok=1;
                    else if (secset[dict[i].first[0]-'A']==1) ok=1;
                 if (ok==1) {
                    for (int j=0; j<n; j++)
                        for (int k=0; k<n; k++)
                            if (ctext[j*n+k]==dict[i].first[0]) {
                               cntapp=0;
                               v[j][k]=1;
                               dfs(dict[i].first,1,j,k,w);
                               v[j][k]=0;
                               if (cntapp!=0 && outa==0) {
                                  fprintf(debug,"p:%d %s is found %d times\n",w,dict[i].first.c_str(),cntapp);
                                  if (dict[i].first[1]=='.') newset[26]=1;
                                     else newset[dict[i].first[1]-'A']=1;
                                  outa=1;
                                  }
                               }
                    }
