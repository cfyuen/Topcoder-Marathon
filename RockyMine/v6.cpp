...
struct Pathway {
	...
	bool operator < (Pathway const &a) const {
		return a.rec.length()<rec.length();
	}
};
...
vector<Route> rou;
Coor pick[300];
int pcnt=0,opn[300],clos[300],forb[60][300][300];
int mxmv[30];
string pd[300][300];
Pathway pat;
	...
	public:
		...
		void getgoldpos () {
			...
			for (int k=0; k<bfin; k++) {
	            ...
	            for (int i=max(0,bf.x-4); i<=min(n-1,bf.x+4); i++)
	                for (int j=max(0,bf.y-4); j<=min(m-1,bf.y+4); j++)
	                    if (reach[i][j]<=10 && reach[i][j]>=1 && g[i][j].rock==0 && g[i][j].rock+b[fin[k].typ].eff[i-sx][j-sy]>0 && g[i][j].gold>0) {
							pick[pcnt].x=i; pick[pcnt].y=j;
							g[i][j].gold=0;
							opn[pcnt]=k;
							pcnt++;
		...
		pair<Coor,string> greedyman () {
			...
			while (!pq.empty()) {
				pat=pq.top();
				fprintf(debug,"pq(%d)= (%d,%d): %d/%d  %s\n  ",pq.size(),pat.c.x,pat.c.y,pat.bp,pat.be,pat.rec.c_str());
				for (int i=0; i<pcnt; i++)
				...
				int ok=1;
				for (int i=0; i<pcnt; i++)
					if (pat.gold[i]==-1 && pat.be>clos[i]) { ok=0; break; }
				if (ok==0) {
					fprintf(debug,"Impossible\n");
					continue;
				}
				goldbfs(pat.be*2);
				if (pat.bp!=bfin) bombbfs(pat.be*2);
			}
			return make_pair(fin[0].c,"");
