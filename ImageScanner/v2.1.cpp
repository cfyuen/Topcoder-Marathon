				...
				if (scaned[i]==1) {
					for (int j=0; j<M; j++)
						if (ret[i][j]=='1' && (j==0 || (j>0 && ret[i][j-1]=='0')) && epn[i][j]==0) {
							...
							for (int k=0; k<pref.size(); k++) {
								...
								if (pref[k].sx<=i && pref[k].sy<=j && pref[k].sx+pn-1>=i && pref[k].sy+pm-1>=j) {
									...
									if (pref[k].mat>best) {
										...
										if (bind>=0 && dif[hash(pref[bind].a,pref[bind].b,pref[bind].sz)][hash(pref[k].a,pref[k].b,pref[k].sz)]>20) secbest=best;
										best=pref[k].mat;
										bind=k;
									}
							...
							if (best/secbest>=3 || best-secbest>20) {
								confirm(bind);
							}
							if (best>10) {
								for (int k=0; k<pref.size(); k++) {
									...
									if (pref[k].sx<=i && pref[k].sy<=j && pref[k].sx+pn-1>=i && pref[k].sy+pm-1>=j) {
										...
										if (pref[k].mat*3<best) {
											useh[hashLID(pref[k])]=0;
											pref.erase(pref.begin()+k);
											k--;
										}
									}
								}
