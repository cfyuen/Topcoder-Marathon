...
int n,v[100][100],cntapp,maxpos,curpos;
string ctext;
vector<int> delta;
FILE *debug;
                ...
                if (i==-1 || word[i]=='.') {
                   for (int j=0; j<MaxSearch; j++) {
                       ...
                       if (ok==1) {
                          ...
                          //fprintf(debug,"dfs: %s %s pos: %d  x,y=%d,%d  ind: %d\n",dict[j].first.c_str(),nword.c_str(),1,x,y,ind+word.length()-1);
                          dfs(nword,1,x,y,ind+word.length()-1);
                          }
                       }
                   }
            return;
