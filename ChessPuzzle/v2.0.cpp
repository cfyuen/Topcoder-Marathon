          ...
          if (!seq.empty()) {
             ...
             if (K>1) {
                if (K*bd.size()*bd[0].size()-removed<=max(250,2*(int)bd.size()*(int)bd[0].size())) {
                   ...
                   */
                   if (tn<timelim/2.0 && K*bd.size()*bd[0].size()-removed>250) {
                      if (bd.size()*bd[0].size()<=120) evalim=150;
                      if (bd.size()*bd[0].size()>120 && bd.size()*bd[0].size()<=180) evalim=120;
                      if (bd.size()*bd[0].size()>180) evalim=80;
                      }
                      else if (tn<timelim*2.0/3 && K*bd.size()*bd[0].size()-removed<200) evalim=220-(int)((K+8)*(bd.size()*bd[0].size())/25.0);
                      else if (tn<timelim*0.9343) {
                              if (K*bd.size()*bd[0].size()-removed>100) evalim=100;
                                 else evalim=120;
                              }
                      else evalim=75;
                   fprintf(stderr,"evalim: %d (%0.4lf)\n",evalim,tn); fflush(stderr);
                   if (bd.size()*bd[0].size()-removed<=bd.size()*bd[0].size()) res=nextmovefinal(1,55,evalim);
                   ...
                   */
                   ...
                   if (tn<timelim*2.0/3) evalim=400-(int)(sqrt(K*bd.size()*bd[0].size())*4.5);
                      else evalim=100;
                   fprintf(stderr,"evalim: %d (%0.4lf)\n",evalim,tn); fflush(stderr);
