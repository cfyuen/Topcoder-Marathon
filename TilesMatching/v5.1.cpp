...
v4.2: discard when after placing, something is 0 and nowd=d
      change scoring, work best for n>=6 and n<=8
v4.3: mixture of 4.2 and 4.1
v5: todo: try to maximise n=4,5 score by making tiles go away together
    todo: try to count excess tiles outside top 3 rows/cols
*/
/*
...
*/
FILE *debug,*result;
int debugmsg=0,localtest=0,st=1001,end=2000;
...
bool revcmp (pair<double,int> a,pair<double,int> b) {
...
class TilesMatching {
   public:
      double scoring () {
         double score=0;
         int topbd[15][15];
         memset(topbd,0,sizeof(topbd));
         //calculate row and col score
         ...
         for (int i=0; i<2; i++)
             for (int j=0; j<s; j++) {
                 topbd[row[i].second][j]=2;
                 topbd[j][col[i].second]=2;
                 }
             
             for (int j=0; j<s; j++) {
                 topbd[row[2].second][j]=1;
                 topbd[j][col[2].second]=1;
                 }
         double occu=0;
         int space=0;
         for (int i=0; i<s; i++)
             for (int j=0; j<s; j++) {
                 if (board[i][j].c!=-1 && topbd[i][j]==0) occu+=1;
                 if (board[i][j].c!=-1 && topbd[i][j]==1) occu+=0.5;
                 if (board[i][j].c==-1) space++;
                 }
         if (space<(s*s)/2) score/=2;
         if (space<(s*s)/3) score/=2;
         fprintf(stderr,"%0.4lf\n",score);
         score+=1500*pow((s*s-6*s+9-occu)/(s*s-6*s+9),4.0);
         if (space<=s*s*5/6) score+=24000*pow((space)*1.0/(s*s*5/6),4.0);
            else score+=24000;
         fprintf(stderr,"%0.4lf\n",score);
         if (debugmsg==1) {
            fprintf(stderr,"%0.1lf\n",occu);
            fprintf(stderr,"%0.4lf\n",score);
            }
         //limited tile
         double okt=n*1.0*n;
         ...
         if (debugmsg==1)
            fprintf(stderr,"%0.4lf %0.4lf\n",okt,pow(okt/(n*1.0*n),((d-nowd)*1.0/d+1)*5.0));
