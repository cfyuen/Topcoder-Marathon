	...
	public:
		...
		void placebomb (Coor at,Coor tar,int tgc,int tgd) {
			...
			*/
			...
			int mindis=1000,para=100000;
			for (int i=0; i<n; i++)
			    for (int j=0; j<m; j++)
					if (reach[i][j]>=1) {
						mindis=min(abs(i-tar.x)+abs(j-tar.y),mindis);
						para=min(para,reach[i][j]+(abs(i-tar.x)+abs(j-tar.y))*2);
					}
			...
			for (int i=0; i<n; i++)
				for (int j=0; j<m; j++) {
					//if (reach[i][j]>=1 && reach[i][j]+(abs(i-tar.x)+abs(j-tar.y))*2<=para+20) fprintf(debug," (%d,%d) : %d\n",i,j,reach[i][j]+(abs(i-tar.x)+abs(j-tar.y))*2);
				    if (reach[i][j]>=1 && reach[i][j]+(abs(i-tar.x)+abs(j-tar.y))*2<=para+6) {
						pt[pn].x=i; pt[pn].y=j;
						pn++;
					}
				}
