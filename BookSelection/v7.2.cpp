...
int hisco=0,use[1400],tots[30],totcnt[30];
map<int,int> done;
double aver[30];
vector<int> bestret,besth,indsco[30];
vector<pair<int,vector<int> > > bestcand,minorcand;
int used[1400],hlimit;


//delete when submit
FILE *debug,*result;
double tlim1_1=2.1,tlim1_2=4.0,tlim1_2f=4.5,tlim2=5.5,tlim2e=5.5,tlim2f=5.75,tout=5.9,mtlim=0.75;
...
/* have it when submit
timeval sttime;
double tlim1_1=0.7,tlim1_2=1.333,tlim1_2f=1.5,tlim2=1.85,tlim2e=1.85,tlim2f=1.92,tout=1.95,mtlim=0.25;
...
class BookSelection {
   public:
      ...
      */
      vector<int> nearbinpack (vector<int> shh,double div) {
          sort(shh.begin(),shh.end());
          maxpershelf=(int)(1400*(1-div));
          memset(used,0,sizeof(used));
          vector<int> bret,wid;
          for (int i=0; i<b.size(); i++) bret.push_back(-1);
          tb=b;
          ...
          for (int i=0; i<shh.size(); i++) {
              ...
              tvv=0;
              for (int j=0; j<b.size(); j++) {
                  if (tb[j].h>shh[i] || (wid[i]>W*div && runtime()<=tout)) break;
                  if (wid[i]+tb[j].w<=W && used[tb[j].ind]==0) {
                     wid[i]+=tb[j].w;
                     bret[tb[j].ind]=i;
                     tvv+=tb[j].v;
      ...
      void minoradjust (vector<int> orih) {
          vector<int> nowh,tret;
          ...
          while (!mpq.empty() && runtime()<tlim2) {
              ...
              bestcand.push_back(make_pair(scor,nowh));
              if (scor>hisco) {
                 bestret=tret;
                 besth=nowh;
                 hisco=scor;
       ...
       vector<int> majorheight (vector<int> geth,double timelim) {
          ...
          while (!pq.empty() && runtime()<timelim) {
              ...
              if (ttt>hisco-200) {
                 ...
                 ttt=scor;
                 if (scor>hisco-20) {
                    bestcand.push_back(make_pair(scor,selh));
                    }
                 if (scor>hisco) {
                    bestret=tret;
                    besth=selh;
                  ...
                  if (at & (1<<i))
                     for (int j=0; j<geth.size(); j++) 
                         if (at & (1<<j)) {}
                            ...
                            if (nsum<=H && (done.find(nat-(1<<nleast))==done.end() || scor-done[nat-(1<<nleast)]>300)) {
              ...
              for (int i=0; i<geth.size(); i++)
                  if (at & (1<<i)) {
                     ...
                     if (nsum<=H && (done.find(nat-(1<<max(i,least)))==done.end() || scor-done[nat-(1<<max(i,least))]>100)) {
                        done[nat-(1<<max(i,least))]=scor;
                        pq.push(make_pair(scor,nat));
                        }
                     }
              }
          fprintf(debug,"operation = %d (%d)\n",opr,bestcand.size());
      ...
      void select () {
          ...
          vector<int> candh=findheight(),tret;
          ...
          sort(bestcand.begin(),bestcand.end());
          int trylim=10;
          for (int i=0; i<bestcand.size() && runtime()<tlim1_2f && i<trylim; i++) {
              if (i>0 && bestcand[bestcand.size()-i].first==bestcand[bestcand.size()-i-1].first) {
                 if (bestcand[bestcand.size()-i].second==bestcand[bestcand.size()-i-1].second) {
                    trylim++; continue;
                    }
                 }
              tret=nearbinpack(bestcand[bestcand.size()-i-1].second,0.9);
              int scor=score(tret);
              fprintf(debug,"Final Cand: ");
              for (int j=0; j<bestcand[bestcand.size()-i-1].second.size(); j++)
                  fprintf(debug,"%d ",bestcand[bestcand.size()-i-1].second[j]);
              fprintf(debug," = %d (%d) -- %d\n",scor,bestcand[bestcand.size()-i-1].first,hisco);
              bestcand[bestcand.size()-i-1].first=scor;
              minorcand.push_back(bestcand[bestcand.size()-i-1]);
              if (scor>hisco) {
                 bestret=tret;
                 hisco=scor;
                 besth=bestcand[bestcand.size()-i-1].second;
                 }
              }
          //fprintf(stderr,"Time 1_2f: %0.4lf\n",runtime()); fflush(stderr);
          bestcand.clear();
          sort(minorcand.begin(),minorcand.end());
          vector<int> th;
          for (int i=0; i<minorcand.size() && runtime()<tlim2e; i++) {
              tlim2=min(tlim2e,runtime()+mtlim);
              th=minorcand[minorcand.size()-i-1].second;
              minoradjust(th);
              }
          
          if (!bestcand.empty()) sort(bestcand.begin(),bestcand.end());
          trylim=15;
          for (int i=0; i<bestcand.size() && runtime()<tlim2f && i<trylim; i++) {
              if (i>0 && bestcand[bestcand.size()-i].first==bestcand[bestcand.size()-i-1].first) {
                 if (bestcand[bestcand.size()-i].second==bestcand[bestcand.size()-i-1].second) {
                    trylim++; continue;
                    }
                 }
              tret=nearbinpack(bestcand[bestcand.size()-i-1].second,0.8);
              int scor=score(tret);
              fprintf(debug,"Final Cand: ");
              for (int j=0; j<bestcand[bestcand.size()-i-1].second.size(); j++)
                  fprintf(debug,"%d ",bestcand[bestcand.size()-i-1].second[j]);
