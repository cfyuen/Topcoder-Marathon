...
**must divide help in row and damage in board
*/
/*
...
*/
FILE *debug,*result;
int debugmsg=0,localtest=1,st=1001,end=2000;
...
class TilesMatching {
   public:
      double scoring (int clear) {
         ...
         if (clear==1) {
            if (space>(s*s*5)/6) score=1000;
               else score=1000*pow(1.01,((s*s*5)/6-space)*2+10.0);
            }
         if (clear==2) {
            if (space>(s*s*5)/6) score=1300;
               else score=1000*pow(1.02,((s*s*5)/6-space)*2+20.0);
            }
         //fprintf(stderr,"---%0.4lf ",score);
         ...
         //score*=pow(okt/(n*1.0*n),((d-nowd)*1.0/d+1)*7.0);
         /*
         if (d==nowd) score*=pow(okt/(n*1.0*n),(n-2)*2);
            else score*=pow(okt/(n*1.0*n),((4-nowd)*1.0/2.5+0.5)*4.0);
         */
         for (int i=0; i<n; i++)
             for (int j=0; j<n; j++) {
                 //if (numcho[i][j]==0) score/=(1+(1.0/n)*(1.0/n)*10-nowd*0.02);
                 if (numcho[i][j]==0) score/=1.25-nowd*1.0/d*0.1-0.001*n*n;
                 if (numcho[i][j]==1 && space<=s*s*4/5) score/=1.02;
                 if (numcho[i][j]==2 && space<=s*s*2/3) score/=1.005;
                 }
         if (debugmsg==1)
      ...
      string placeNormal (string tile) {
         ...
         //fprintf(stderr,"R: %0.4lf\n",bestsco); fflush(stderr);
         if (bestsco>25000)
            return phase(tcur,bestx,besty);
         if (tcur.c<elimc[n] && nowd>1) {
            nowd--;
            return "DISCARD";
            }
         if (tcur.s<elims[n] && nowd>1) {
            nowd--;
            return "DISCARD";
            }
         if (bestsco>000 || (bestsco>1e-9 && nowd==0) || bestsco>nowscore) {
            return phase(tcur,bestx,besty);
            }
         if (nowd>0) {
            nowd--;
...
int main () {
    ...
    MersenneTwister r;
    result=fopen("result v5.2.txt","w");
