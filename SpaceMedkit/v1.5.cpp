...
2.    In phrase 1 (v1.3 bin packing)
      ...
      Interphrase
         try to include the most urgent resource (among the expensive ones)
            v1.5 to try multiple resources at a time
      In phrase 2
...
//     totres      Rid amount
vector<vector<pair<int,double> > > bin;

int cntop;

//delete when submit
FILE *debug,*result;
int local=1,stseed=62,endseed=100;
   ...
   public:
      ...
      void phrase1 () { 
         ...
         for (int i=0; i<handle.size(); i++) {
             ...
             handle[i].first=cost;
             //int putbin=0;
             double costlimit=0.2;
             vector<pair<int,double> > totv;
             for (int j=0; j<handle[i].second.second.size(); j++) {
                 //     to make sure this is unique
                 if (keyres[handle[i].second.second[j]]==1 && totres[handle[i].second.second[j]]*rcost[handle[i].second.second[j]]>costlimit) {
                    //putbin++;
                    totv.push_back(make_pair(handle[i].second.second[j],totres[handle[i].second.second[j]]));
      ...
      vector <string> getMedkit(vector <string> ar, vector <string> rr, vector <string> mis, double _P, double _C) {
         ...
         parseEvent(rr);
         parseMis(mis);
         //target=(int)(P*noofmis); //version 1.4
         target=(int)(P*noofmis)-(int)(P*2000); //version 1.4a
         ...
         do {
            ...
            manual=-1; freq=1e9;
            //               bin[][]                 num  cost
            vector<pair<vector<pair<int,double> >,pair<int,double> > > uniq;
            // not yet bin pack
            int tcnt=0;
            sort(bin.begin(),bin.end());
            for (int i=0; i<bin.size(); i++) {
                //for (int j=0; j<bin[i].size(); j++)
                //    fprintf(debug,"R%d-%0.4lf  ",bin[i][j].first,bin[i][j].second);
                if (i>0 && bin[i]!=bin[i-1]) {
                   amount=0;
                   for (int j=0; j<bin[i-1].size(); j++)
                       amount+=rcost[bin[i-1][j].first]*bin[i-1][j].second;
                   uniq.push_back(make_pair(bin[i-1],make_pair(tcnt,amount)));
                   tcnt=1;
                   }
                   else tcnt++;
                //fprintf(debug,"\n");
            ...
            amount=0;
            for (int j=0; j<bin[bin.size()-1].size(); j++)
                amount+=rcost[bin[bin.size()-1][j].first]*bin[bin.size()-1][j].second;
            uniq.push_back(make_pair(bin[bin.size()-1],make_pair(tcnt,amount)));
            for (int i=0; i<uniq.size(); i++) {
                fprintf(debug,"%d(%0.4lf): ",uniq[i].second.first,uniq[i].second.second);
                for (int j=0; j<uniq[i].first.size(); j++)
                    fprintf(debug,"R%d-%0.4lf  ",uniq[i].first[j].first,uniq[i].first[j].second);
                fprintf(debug,"\n");
                }
            int stillneed=alluntreat+resolved-target;
            if (stillneed>1000) {
               for (int i=0; i<uniq.size(); i++) {
                   if (uniq[i].second.second/uniq[i].second.first<freq) {
                      freq=uniq[i].second.second/uniq[i].second.first;
                      manual=i;
                      }
                   }
               }
               ...
               for (int i=0; i<uniq.size(); i++) {
                   if (uniq[i].second.second<freq) {
                      freq=uniq[i].second.second;
                      manual=i;
                      }
                   }
               }
            fprintf(debug,"Chosen:  ");
            for (int i=0; i<uniq[manual].first.size(); i++) {
                fprintf(debug,"R%d-%0.4lf  ",uniq[manual].first[i].first,uniq[manual].first[i].second);
                nowcost+=rcost[uniq[manual].first[i].first]*(uniq[manual].first[i].second-curruse[uniq[manual].first[i].first]);
                curruse[uniq[manual].first[i].first]=max(uniq[manual].first[i].second,curruse[uniq[manual].first[i].first]);
                }
            fprintf(debug,"\n");
            bin.clear();
...
int main () {
    ...
    vector<string> RET;
    if (nowseed>=stseed) RET=SMK.getMedkit(aR,rR,M,inP,inC);
    //evaluate
    printf("Evaluate\n");
