...
int bplace=0,bbsco[3000],bfin=0;
double timenow=0;
	...
	public:
		...
		void placebomb (Coor at,Coor tar,int tgc,int tgd) {
			...
			*/
			...
			int mindis=1000,para=10000;
			for (int i=0; i<n; i++)
			    for (int j=0; j<m; j++)
					if (reach[i][j]>=1) {
						mindis=min(abs(i-tar.x)+abs(j-tar.y),mindis);
						para=min(abs(i-tar.x)+abs(j-tar.y)+reach[i][j],para);
					}
			fprintf(debug,"Dist = %d Para %d\n",mindis,para); fflush(debug);
			...
			for (int i=0; i<n; i++)
				for (int j=0; j<m; j++)
				    if (reach[i][j]>=1 && abs(i-tar.x)+abs(j-tar.y)+reach[i][j]<=para+3) {
						pt[pn].x=i; pt[pn].y=j;
						pn++;
					}
		...
		pair<Coor,string> saferoute (Coor at,int bbid) {
			...
			pick[pcnt]=bf; pcnt++;
			fill(bf);
            for (int i=max(0,bf.x-4); i<=min(n,bf.x+4); i++)
                for (int j=max(0,bf.y-4); j<=min(m,bf.y+4); j++)
